...
def process_and_visualize(file1, file2, threshold):
    ...
    # ユークリッド距離の計算（X, Y, Z 座標の差の2乗和の平方根）
    merged_data['Distance_diff'] = np.sqrt(
        (merged_data['X_1'] - merged_data['X_2']) ** 2 +
        (merged_data['Y_1'] - merged_data['Y_2']) ** 2 +
        (merged_data['Z_1'] - merged_data['Z_2']) ** 2
    )

    # 閾値判定（ユークリッド距離が閾値を超えるか）
    merged_data['Exceeds_Threshold'] = merged_data['Distance_diff'] > threshold
    ...
    pcd_above_2 = create_point_cloud(above_threshold_2, [0, 1, 0])  # 黄
...
def select_files_and_run():
    ...
    tk.Label(root, text="閾値を入力してください（距離）:").pack(pady=5)
